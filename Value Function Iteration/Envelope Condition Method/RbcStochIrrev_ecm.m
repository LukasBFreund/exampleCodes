%%=========================================================================
% RBC model without labor, irreversible capital
...
deltta = 0.01;                         % capital depreciation rate  
sigma_z = 0.025;                         % standard deviation, technology shock; changed this to 0.02 relative to problem set
...
ECMDVF = 0;                             % if ==0, iterature on value function, if ==1, iterate on derivative of VF
...
while Metric > ConvTol
    ...
    vMU = vVDer./(1-deltta+A*alppha*vZTGrid.*vKTGrid.^(alppha-1));
                                                        % U_c (marginal utility) from
                                                        % envelope condition (U_c=V_k/R)
    vC = vMU.^(-1/gamma);                               % Recover C
    vKPrime = fProduction(vKTGrid,vZTGrid)+(1-deltta)*vKTGrid-vC;
                                                        % Recover implied K'
    
    vUnconstIdx = (vKPrime>(1-deltta)*vKTGrid);           % takes on the value 0 if constrained 
    % Add consideration of irreversibility constraint
    if vKPrime>(1-deltta)*vKTGrid
        vMu = 0;
    else
        vKPrime=vUnconstIdx.*vKPrime+(1-vUnconstIdx).*((1-deltta)*vKTGrid);
        vC = vUnconstIdx.*vC+(1-vUnconstIdx).*fProduction(vKTGrid,vZTGrid);
    end                                                        
    ...
    if ECMDVF==0
        ...
        vVDerPrime=zeros(TGridNum,GHNodesNum);  % pre-allocate
         for iG = 1:GHNodesNum
...
for iT = 1:T
 % Solve under assumption that constraint isn't binding
 vMuSim(iT,1) = 0;
 ...
 vUnconstIdxSim(iT,1) = (vInvSim(iT,1)>0);
 if vUnconstIdxSim(iT,1) == 0                % if constraint is binding
     vInvSim(iT,1) = 0;
     vKSim(iT+1) = (1-deltta)*vKSim(iT,1);
     vCSim(iT,1) = A*vZSim(iT,1).*vKSim(iT,1).^(alppha);
     vMuSim(iT,1) = vCSim(iT,1).^(-1/gamma) - MUtil;
end
end
...
set(gca,'FontSize',12,'fontname','times')
subplot(2,3,5);
plot(vMuSim(end-TSimPlot:end),'linewidth',1.6,'color','k');
ylabel('Constraint Multiplier, \mu','FontSize',12,'fontname','times')
xlabel('Time (quarters)','FontSize',12,'fontname','times')
set(gca,'FontSize',12,'fontname','times')
